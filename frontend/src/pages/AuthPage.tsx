...
export default function AuthPage() {
  ...
  const handleSignup = async (form: SignupFormData, reset: () => void) => {
    ...
    try {
      ...
      setSignupMessage({
        tone: "success",
        text: "Registrierung erfolgreich! Bitte jetzt anmelden.",
      });
      reset();
    } catch (err) {
  ...
  const handleLogin = async (
    ...
    try {
      ...
      setToken(newToken);
      setLoginMessage({ tone: "success", text: "Anmeldung erfolgreich. Profil wird geladen..." });
      reset();
  ...
  const handleUpdate = async (data: UpdateProfileInput) => {
    ...
    if (!token) {
      setUpdateMessage({ tone: "error", text: "Bitte zuerst anmelden." });
      return;
    }
    setBusy(true);
    try {
      ...
      setUpdateMessage({
        tone: "success",
        text: "Daten gespeichert. Angebote können erstellt werden.",
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unbekannter Fehler";
        ...
        <div className="layout__column">
          ...
          {token ? (
            ...
            <Card title="Fahrzeugverwaltung">
              <p className="muted">Bitte anmelden, um Fahrzeuge zu verwalten.</p>
            </Card>
          )}
        </div>
        <div className="layout__column">
